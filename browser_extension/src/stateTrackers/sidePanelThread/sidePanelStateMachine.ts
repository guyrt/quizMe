...
import {
  SinglePageDetailsChangeMessage,
  SinglePageDetails,
  ChromeMessage,
  MaybeSinglePageDetails,
  SinglePageDetailsErrorState,
} from '../../interfaces'
...
class SidePanelFiniteStateMachine {
  ...
  public async updateState(singlePage: SinglePageDetails) {
    ...
    if (
      singlePage.uploadState == 'inprogress' ||
      singlePage.uploadState == 'notstarted'
    ) {
  ...
  public async handleError(singlePage: SinglePageDetailsErrorState) {
    if (singlePage.error == 'auth') {
      ...
      singlePage.error == 'cachemiss' ||
      singlePage.error == 'nopage'
    ) {
  ...
  public async triggerCheck() {
    ...
    chrome.runtime.sendMessage(
      { action: 'fa_getCurrentPage', payload: {} },
      (_domFacts: MaybeSinglePageDetails) => {
...
chrome.runtime.onMessage.addListener(
  (message: SinglePageDetailsChangeMessage, sender) => {
