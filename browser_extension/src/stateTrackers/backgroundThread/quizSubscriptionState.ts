...
class QuizHistoryState {
  ...
  public async getLatestQuizHistory(): Promise<QuizHistory | undefined> {
    if (
      this.quizHistory != undefined &&
      Object.keys(this.quizHistory).length > 0
    ) {
      return this.quizHistory
    }

    const storedHistory = (await chrome.storage.session.get('quizHistory')) as
      | QuizHistory
      | undefined
