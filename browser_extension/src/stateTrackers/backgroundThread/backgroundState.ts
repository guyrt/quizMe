...
class BackgroundState {
  ...
  public async uploadPage(tabId: number, domSummary: DomShape) {
    ...
    if (!(await this.shouldOperateOnPage(record))) {
      ...
      pageDetailsStore.setPageDetails(
        record.key,
        { ...record, uploadState: 'donotprocess' },
        true,
      )
      return
    }
    ...
    const uploadableDom = this.buildUploadableDom(
      domSummary,
      record.guid,
    ...
    this.uploadPromises[record.key]
      .then(x => {
        console.log(
          `Upload complete for tab ${tabId} url ${domSummary.url.href}`,
        )
        pageDetailsStore.setPageDetails(
          record.key,
          { ...record, uploadState: 'completed', uploadedDom: x },
          true,
        )
  ...
  public async uploadNewVersionSamePage(tabId: number, domSummary: DomShape) {
    ...
    this.uploadPromises[tabId].then(() => {
      ...
      this.uploadPromises[record.key] = sendDomPayloadUpdate(
        token,
        uploadableDom,
      )
    })
  ...
  public async getOrCreateAQuiz(
    ...
    getAQuiz(record.uploadedDom, forceReload).then(newDom =>
      this.updatePayloadAndReturnQuiz(record, newDom),
    )
    return this.setQuiz(record, { status: 'building' })
  }
  ...
  private updatePayloadAndReturnQuiz(
    record: SinglePageDetails | undefined,
    uploadedDom: UploadedDom | undefined,
  ): Quiz {
    if (record != undefined && uploadedDom != undefined) {
      // update the dom to include the quiz.
      pageDetailsStore.setPageDetails(
        record.key,
        { ...record, uploadedDom: uploadedDom },
        true,
      )
    }
  ...
  private async shouldOperateOnPage(
    response: SinglePageDetails,
  ): Promise<boolean> {
    if (
      (await sharedState.getDomainBlockList()).some(x =>
        response.url.host.endsWith(x),
      )
    ) {
      return false
    }
  ...
  private async getOrCreatePageDetails(
    key: number,
    response: DomShape,
  ): Promise<MaybeSinglePageDetails> {
    let pageDetail = await pageDetailsStore.getPageDetails(key)
    ...
    const urlMismatch =
      missingKey ||
      (pageDetail as SinglePageDetails).url.href != response.url.href
