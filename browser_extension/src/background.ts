...
chrome.runtime.onMessage.addListener(
  (message: QuizResponseMessage, sender, sendResponse) => {
    ...
    if (message.action === 'fa_pageLoaded') {
      ...
      ;(async () => {
        ...
        if (loadedUrl == 'unknown') {
          ...
          chrome.tabs.query(
            { currentWindow: true, url: loadedUrl },
            function (tabs) {
              handleTabs(tabs, true)
            },
          )
      ...
      chrome.tabs.sendMessage(
        ...
        x =>
          handleFAAccessDOMMessage(tId, x, message.action === 'fa_pageLoaded'),
      )
      ...
      ;(async () => {
        chrome.tabs.query(
          ...
          function (tabs) {
            ...
            if (activeTabId !== undefined) {
              const q = backgroundState.getOrCreateAQuiz(
                activeTabId,
                message.payload['forceReload'] ?? false,
              )
              sendResponse(q)
            } else {
...
function handleFAAccessDOMMessage(
  tabId: number,
  response: DomShape,
  firstUpload: boolean,
) {
  console.log(
    `Background received dom. TabId: ${tabId}, isFirst: ${firstUpload} response:`,
    response,
  )
  if (firstUpload) {
...
function argMax<T extends Record<K, number>, K extends keyof any>(
  listOfObjects: T[],
  key: K,
): T {
