...
class Parser:
    def parse_response(self, response: str) -> List[Response]:
        ...
        # noop
        return [Response(content=response, source="generated")]


class WhatEventParser(Parser):
    def parse_response(self, response: str) -> List[Response]:
        lines = [lin for lin in response.splitlines() if lin]
        assert len(lines) == 2
        ...
        responses = [
            Response(content=quote, source="quote"),
            Response(content=summary, source="generated"),
        ]
        return responses
...
class JsonListParse(Parser):
    ...
    def parse_response(self, response: str) -> List[Response]:
        # TODO: build recovery
        try:
            response = self._corrector.run(
                response
            )  # try to correct the problem, or pass through.
            ...
            raise e
            # hey old man, you should try pycharms debugger
            # import pdb
            #
...
class JsonListOfStringsParser(JsonListParse):
    def _parse_internal(self, data: List[Any]):
        return [Response(content=line, source="generated") for line in data]
...
class DocQuestionsParse(JsonListParse):
    def _parse_internal(self, data) -> List[Response]:
        ...
        for elt in data:
            responses.extend(
                [
                    Response(content=elt["question"], source="generated"),
                    Response(content=elt["answer"], source="generated"),
                    Response(content=elt["source"], source="quote"),
                ]
            )
...
class GetEntities(JsonListParse):
    def _parse_internal(self, data) -> List[Response]:
        ...
        for elt in data:
            responses.append(
                Response(content=f"{elt['name']}: {elt['reason']}", source="generated")
            )
