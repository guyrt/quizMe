...
def gpt_extract(raw_docextracts: List[int], doc_cluster_id):
    ...
    for doc_file in DocumentFile.objects.filter(active=1).filter(
        document__pk=doc_cluster_id
    ):
        ...
        try:
            for raw_docextract in (
                raw_docextracts
            ):  # todo - consider a change here. You'll need to regroup prompts.
...
def create_known_facts_from_cluster(doc_cluster_id: int):
    ...
    prompts = list(
        PromptResponse.objects.filter(
            document_inputs__docfile__document=doc_cluster
        ).filter(document_inputs__active=1)
    )
