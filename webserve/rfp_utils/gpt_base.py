...
class BasePromptRunner:
    ...
    def execute(self, doc_extract_id: int) -> Iterator[PromptResponse]:
        ...
        logger.info(
            "run intelligence on doc %s with %s chunks.",
            doc_extract_id,
            len(content_chunks),
        )
        ...
        for prompt in self._build_prompts():
            ...
            for chunk in filtered_chunks:
                raw_results = self._run_prompt_on_chunk(
                    prompt, chunk
                )  # run on chunk no side effect
                raw_parsed_results = self._process_single_result(
                    doc, prompt, raw_results, len(content_chunks)
                )  # save sub prompts

                for rpr in raw_parsed_results:
                    logger.info(
                        f"Created PromptResponse {rpr.id} from {rpr.template_name}@{rpr.template_version} on DocumentExtract {rpr.document_inputs.all()[0].id}"
                    )
                    yield rpr
    ...
    def _process_single_result(
        self, doc: DocumentExtract, prompt: Prompt, results: List[str], num_chunks: int
    ) -> List[PromptResponse]:
    ...
    def _run_prompt_on_chunk(self, prompt: Prompt, doc: str) -> List[dict]:
        ...
        messages.append(
            asdict(PromptCell(role="assistant", content=raw_response_d["response"]))
        )
