...
class QuizGenerator:
    ...
    def create_quiz(self, raw_doc: RawDocCapture, quiz_id: int) -> Optional[SimpleQuiz]:
        ...
        SimpleQuiz.objects.filter(id=quiz_id).update(
            status=SimpleQuiz.QuizStatus.Building
        )
        ...
        ConsumerPromptTrack.objects.create(
            user=raw_doc.user,
        ...
        with transaction.atomic():
            ...
            SimpleQuiz.objects.filter(url=raw_doc.url_model).exclude(id=quiz_id).update(
                active=False
            )
    ...
    def _shrink_article(self, article_content: str) -> str:
        ...
        total_allowed_tokens = (
            8000 - self._return_tokens - num_prompt_tokens
        )  # allow 1000 for return.
        ...
        if total_article_size // num_tokens_to_remove < 0.25:
            ...
            section = sections[2]
            delete_ratio = (
                num_tokens_to_remove / self._oai.num_tokens_from_string(section) + 0.02
            )  # fudge factor...
    ...
    def _run_openai(self, article_content: str):
        current = fill_prompt(
            quiz_gen, {"doc_content": article_content, "num_questions": "one"}
        )
