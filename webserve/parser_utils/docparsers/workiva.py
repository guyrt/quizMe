...
class WorkivaParser:
    def parse(self, dom: BeautifulSoup) -> List[str]:
        ...
        while len(stack):
            ...
            if elt.name == "header" and elt.prefix == "ix":
                ...
                table_content, local_structured_data = self._clean_table(
                    elt, context_dict
                )
            ...
            elif elt.name == "span" and (
                "font-size:5.85pt;" in elt.attrs.get("style", "")
            ):
                if any(elt.children):
        ...
        return ParserReturn(
            parsed_doc=[
                s for s in string_elts if s and "amounts in millions" not in s.lower()
            ],
    ...
    def _clean_table(
        self, table: BeautifulSoup, context_table: Dict[str, any]
    ) -> (str, Dict[str, any]):
    ...
    def _parse_structured_tr(self, tr: BeautifulSoup, context_table: Dict[str, any]):
        ...
        for td in (
            tr.children
        ):  # assuming well-formatted here. could verify all children are td.
            if td.name is None:
                continue
                ...
                for ix_elt in ix_elts:
                    ...
                    if structured_data["contextRef"]:
                        structured_data.update(
                            context_table[structured_data["contextRef"]]
                        )
    ...
    def _make_markdown(self, row_list):
        ...
        row_list = [
            {k: v for k, v in r.items() if k in keys_to_keep.keys()} for r in row_list
        ]
        for row in row_list:
            if "instant" in row:
        ...
        col_widths = {
            col_name: max([len(str(r[col_name])) for r in row_list] + [len(col_name)])
            for col_name in keys_to_keep
        }
        ...
        delim_row = {
            col_name: "-" * col_widths[col_name] for col_name in keys_to_keep.keys()
        }

        try:
            return "\n".join(
                [row_fmt.format(**row) for row in [keys_to_keep, delim_row] + row_list]
            )
        except:
            import pdb
