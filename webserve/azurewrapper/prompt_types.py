...
def fill_prompt_cell(cell: PromptCell, context):
    return PromptCell(
        role=cell.role, content=cell.content.format(**context).strip()
    )  # strip b/c trailing spaces degrade perf.
...
def promp_response_from_dict(d) -> PromptResponse:
    p = Prompt(
        ...
        continuations=[
            prompt_cell_from_d(c) for c in d["prompt"].get("continuations", list())
        ],
        temp=d["prompt"]["temp"],
    )
    pr = PromptResponse(
...
def prompt_cell_from_d(d) -> PromptCell:
    pc = PromptCell(role=d["role"], content=d["content"])
    return pc


def responses_from_list(list_of_responses: list[dict[str, str]]) -> List[Response]:
    return [
        Response(content=d["content"], source=d["source"]) for d in list_of_responses
    ]
